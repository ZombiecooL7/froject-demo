...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function seedMailbox() {
    ...
    mailbox = {
      inbox: [
        { id: 'm1', from: 'Linear', addr: 'product@linear.app', subject: 'Еженедельный дайджест продукта', time: '9:41', body: 'Ваша команда отправила 23 задачи на этой неделе. 87% завершено в срок — это лучший показатель за квартал!\n\nТоп задач:\n• Сверстать новый sidebar\n• Календарь: события + Smart Scheduling\n• Дизайн-система v2.0', unread: true, avatar: 'L', color: 'green', label: 'work', summary: 'Команда показала рекордные 87% завершённых задач в срок. Лучший показатель за квартал.', starred: false },
        { id: 'm2', from: 'София Чен', addr: 'sofia.chen@fmail.ru', subject: 'Re: Обзор дорожной карты Q3', time: '8:12', body: 'Спасибо, что прислали презентацию. У меня было несколько мыслей по вопросу 2 — давайте обсудим в пятницу.\n\nКлючевые моменты:\n• Приоритизация Mobile App\n• AI-фичи в Q3\n• Расширение команды на 3 человека', unread: true, avatar: 'С', color: 'purple', label: 'work', starred: true },
        { id: 'm3', from: 'Figma', addr: 'noreply@figma.com', subject: 'Маркус прокомментировал ваш файл', time: 'Вчера', body: 'Маркус прокомментировал файл «Froject — Главный экран»:\n\n«Нравится новое направление на главном экране. Особенно работа с тёмной темой и неоновыми акцентами.»\n\nОткройте Figma, чтобы ответить.', unread: false, avatar: 'F', color: 'purple', label: 'work', summary: 'Маркус оставил положительный комментарий к макету главного экрана.', starred: false },
        { id: 'm4', from: 'Stripe', addr: 'notify@stripe.com', subject: 'Выплата $12,480.00 отправлена', time: 'Вчера', body: 'Ваша выплата уже в пути на ваш банковский счёт.\n\nДетали:\n• Сумма: $12,480.00\n• Назначение: Bank ending in •••• 4242\n• Ожидаемое поступление: 2 рабочих дня', unread: false, avatar: 'S', color: 'blue', label: 'finance', starred: false },
        { id: 'm5', from: 'Vercel', addr: 'no-reply@vercel.com', subject: 'Развёртывание готово для froject-web', time: 'Пн', body: 'Предпросмотр доступен на froject-web-g3f.vercel.app\n\nДеплой готов, тесты пройдены, билд успешен.', unread: false, avatar: 'V', color: 'green', label: 'work', starred: false },
        { id: 'm6', from: 'GitHub', addr: 'notifications@github.com', subject: '[froject/core] PR #482 утверждён', time: 'Пн', body: 'Ваш pull request был одобрен и готов к слиянию.\n\nReviewers: @alex, @maria\n+342 lines, -89 lines\n12 commits', unread: false, avatar: 'G', color: 'yellow', label: 'work', starred: false },
        { id: 'm7', from: 'Команда Froject', addr: 'team@fmail.ru', subject: 'Добро пожаловать в Froject ✦', time: 'Пн', body: 'Спасибо, что выбрали просто работающую почту.\n\nЧто стоит попробовать:\n• Настроить метки и фильтры\n• Включить AI-сводки\n• Подписать партнёрский email', unread: false, avatar: 'F', color: 'green', label: 'work', starred: false },
        { id: 'm8', from: 'Notion', addr: 'updates@notion.so', subject: 'Ваш отчёт за неделю готов', time: 'Вс', body: 'Сводка активности рабочего пространства за неделю.\n\n• 142 правки в документах\n• 38 новых комментариев\n• 12 страниц создано', unread: false, avatar: 'N', color: 'red', label: 'work', starred: false },
      ],
      starred: [],
      sent: [
        { id: 's1', from: s.name || s.login, addr: mailAddrFor(s.login), subject: 'RE: Дедлайн проекта', time: 'вчера, 19:11', body: 'Принято. Сегодня закрою последние задачи и отправлю демо.', unread: false, avatar: (s.name || s.login).charAt(0).toUpperCase(), color: 'green', starred: false },
      ],
      drafts: [
        { id: 'd1', from: 'Черновик', addr: mailAddrFor(s.login), subject: 'Re: Обзор Q3 — мои мысли', time: 'сегодня', body: '(не отправлено)\n\nСпасибо за встречу! Хотел зафиксировать пару моментов...', unread: false, avatar: '✎', color: 'yellow', starred: false },
        { id: 'd2', from: 'Черновик', addr: mailAddrFor(s.login), subject: 'Идея для нового модуля', time: 'вчера', body: '(не отправлено)\n\nДумаю про модуль аналитики команды...', unread: false, avatar: '✎', color: 'yellow', starred: false },
      ],
      archive: [],
      trash: [],
    };
    // populate starred from inbox flagged items
    mailbox.starred = mailbox.inbox.filter(m => m.starred);
    persistMail();
    renderMail();
  }
  ...
  function renderMail() {
    if (!mailbox) return;
    // ensure all folders exist (back-compat for old saved data)
    ['inbox','starred','sent','drafts','archive','trash'].forEach(f => { if (!mailbox[f]) mailbox[f] = []; });
    // refresh starred from inbox
    mailbox.starred = mailbox.inbox.filter(m => m.starred);

    // counts
    ['inbox','starred','sent','drafts','archive','trash'].forEach(f => {
    ...
    // folder name + counter
    const fName = { inbox:'Входящие', starred:'Избранные', sent:'Отправленные', drafts:'Черновики', archive:'Архив', trash:'Корзина' }[currentFolder];
    ...
    if (listEl) {
      listEl.innerHTML = items.map(m => `
        <button class="mail-row ${m.unread ? 'unread' : ''} ${currentMailId === m.id ? 'active' : ''}" data-mail="${m.id}">
          <div class="mr-avatar av-${m.color || 'green'}">${escapeHtmlLocal(m.avatar || m.from.charAt(0))}</div>
          <div class="mr-body">
            <div class="mr-top">
      ...
      `).join('') || '<div class="muted" style="padding:32px 24px;text-align:center">Папка пуста</div>';
    }
  }
  ...
  function openMail(id) {
    ...
    // Sender section
    const fromEl = document.getElementById('mail-r-from');
    if (fromEl) fromEl.textContent = m.from;
    const metaEl = document.getElementById('mail-r-meta');
    if (metaEl) metaEl.innerHTML = 'мне · <span id="mail-r-time">' + escapeHtmlLocal(m.time) + '</span>';
    const avEl = document.getElementById('mail-r-avatar');
    if (avEl) {
      avEl.textContent = m.avatar || m.from.charAt(0);
      avEl.className = 'avatar mail-r-avatar av-' + (m.color || 'green');
    }

    // Label chip
    const labelEl = document.getElementById('mail-r-label');
    if (labelEl) {
      const labelNames = { work: 'Работа', personal: 'Личное', travel: 'Путешествия', finance: 'Финансы' };
      if (m.label && labelNames[m.label]) {
        labelEl.textContent = labelNames[m.label];
    ...
    // AI summary block
    const sumEl = document.getElementById('mail-r-summary');
    const sumTxt = document.getElementById('mail-r-summary-text');
    if (sumEl && sumTxt) {
      if (m.summary) {
        sumTxt.textContent = m.summary;
        sumEl.hidden = false;
      } else {
        sumEl.hidden = true;
      }
    ...
    // Star button state
    const starBtn = document.getElementById('mail-star-btn');
    if (starBtn) starBtn.classList.toggle('starred', !!m.starred);
  ...
  /* ==========================================================
     LIMELIGHT NAV — sliding green bar on sidebar
     ========================================================== */
  const limelight = document.getElementById('nav-limelight');
  const navListMain = document.getElementById('nav-list-main');
  const navListBottom = document.getElementById('nav-list-bottom');

  function moveLimelight() {
    if (!limelight) return;
    // find the active nav-item in EITHER list
    const active = document.querySelector('.sidebar .nav-item.active');
    if (!active || !navListMain) return;
    // compute offsetTop relative to sidebar (not to nav-list)
    ...
    while (el && el !== sidebar) {
      top += el.offsetTop;
      el = el.offsetParent;
      if (el && el.classList && el.classList.contains('sidebar')) break;
    }
    const h = active.offsetHeight;
    limelight.style.height = (h - 8) + 'px';
    limelight.style.top = (top + 4) + 'px';
    requestAnimationFrame(() => limelight.classList.add('ready'));
  }
  // Initial position + observe section changes
  setTimeout(moveLimelight, 50);
  setTimeout(moveLimelight, 250);
  window.addEventListener('resize', moveLimelight);
  document.querySelectorAll('.sidebar .nav-item').forEach(n => {
    n.addEventListener('click', () => setTimeout(moveLimelight, 10));
  });
  // when nav-items get .active class via section switches, re-position
  const navObserver = new MutationObserver(() => moveLimelight());
  if (navListMain) navObserver.observe(navListMain, { attributes: true, subtree: true, attributeFilter: ['class'] });
  if (navListBottom) navObserver.observe(navListBottom, { attributes: true, subtree: true, attributeFilter: ['class'] });

  /* ==========================================================
     MAIL WELCOME BANNER (first visit, demo or new account)
     ========================================================== */
  function maybeShowMailBanner() {
    const banner = document.getElementById('mail-welcome-banner');
    if (!banner) return;
    if (localStorage.getItem('froject_mail_seen') === '1') return;
    const s = currentUser();
    if (!s) return;
    const isDemo = s.login === 'demo';
    const isFresh = s.createdAt && (Date.now() - s.createdAt) < 30 * 60 * 1000; // within 30min
    if (!isDemo && !isFresh) return;
    banner.hidden = false;
    if (window.feather) feather.replace();
  }
  function closeMailBanner(setSeen) {
    const banner = document.getElementById('mail-welcome-banner');
    if (!banner) return;
    if (setSeen) localStorage.setItem('froject_mail_seen', '1');
  ...
  // Trigger banner whenever Mail section becomes active
  const mailSection = document.getElementById('mail');
  if (mailSection) {
    const mailObs = new MutationObserver(() => {
      if (mailSection.classList.contains('active')) maybeShowMailBanner();
    });
    mailObs.observe(mailSection, { attributes: true, attributeFilter: ['class'] });
    // also check at startup if mail is already active
    if (mailSection.classList.contains('active')) setTimeout(maybeShowMailBanner, 400);
  }
  // Wire banner buttons
  document.getElementById('mwb-accept')?.addEventListener('click', () => closeMailBanner(true));
  document.getElementById('mwb-skip')?.addEventListener('click', () => closeMailBanner(false));
  document.getElementById('mwb-close')?.addEventListener('click', () => closeMailBanner(false));
  document.querySelector('#mail-welcome-banner .mwb-backdrop')?.addEventListener('click', () => closeMailBanner(false));

  /* ==========================================================
     NEW MAIL BUTTONS WIRING (compose-side, settings, label, star)
     ========================================================== */
  // New compose button id from redesigned sidebar
  document.getElementById('mail-compose-btn-side')?.addEventListener('click', () => {
    const m = document.getElementById('compose-modal');
    if (m) { m.classList.add('show'); m.setAttribute('aria-hidden', 'false'); }
  });
  // Mail "Настройки" button in sidebar → navigate to settings section
  document.querySelectorAll('[data-section-go]').forEach(b => {
    b.addEventListener('click', () => {
      const target = b.dataset.sectionGo;
      const navEl = document.querySelector(`.sidebar .nav-item[data-section="${target}"]`);
      if (navEl) navEl.click();
    });
  });
  // Label filter clicks — switch to inbox folder filtered by label (visual only for demo)
  document.querySelectorAll('.mail-label').forEach(lb => {
    lb.addEventListener('click', () => {
      document.querySelectorAll('.mail-label').forEach(x => x.classList.toggle('active', x === lb));
      // Switch to inbox if not there
      const inboxBtn = document.querySelector('.mail-folder[data-folder="inbox"]');
      if (inboxBtn && !inboxBtn.classList.contains('active')) inboxBtn.click();
    });
  });
  // Toolbar reply → focus textarea
  document.getElementById('mail-reply-toolbar-btn')?.addEventListener('click', () => {
    document.getElementById('mail-reply-input')?.focus();
  });
  // Star button
  document.getElementById('mail-star-btn')?.addEventListener('click', () => {
    if (!currentMailId || !mailbox) return;
    // find in inbox (starred lives in inbox)
    const m = (mailbox.inbox || []).find(x => x.id === currentMailId);
    if (!m) return;
    m.starred = !m.starred;
    persistMail();
    document.getElementById('mail-star-btn').classList.toggle('starred', m.starred);
    renderMail();
  });
